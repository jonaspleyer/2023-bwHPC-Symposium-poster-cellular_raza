...
\begin{abstract}
    ...
    First, we develop a simple model with cell-type specific interactions which results in the
    well-known cell-sorting phenomenon.
    ...
    We show that values of up to $p=97.78\pm0.14\%$ parallelizable code are achievable.
\end{abstract}
...
In order to provide more details, we focus on two case-studies which were also presented.
...
examples and information about the underlying assumptions and methods which we leveraged in the
development of \lstinline{cellular_raza}.
...
We have shown along the example of cell-sorting in section~\ref{sec:cell-sorting} how
\lstinline{cellular_raza} can be used to model cellular biological systems.
The modeling process is flexible due to the variety of cellular representations which are supported.
Afterwards, we applied the cell-sorting simulation to benchmark the performance of the numerical
backend by solving the specified equations.
The fractional amount of parallelizable code reached values up to $p=97.78\%$ for a workstation
setup.
